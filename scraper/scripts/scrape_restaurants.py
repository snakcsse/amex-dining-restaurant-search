...
# Obtain page links (such that all restaurants can be scraped by scraping restaurants in each page)
def get_page_links(base_url):
    soup = get_soup(base_url)
    if soup:
        totalRestaurant = int(re.sub(r'\D', '',soup.select_one('span.pager-text-total').text)) # extracting #total restaurants, removing "全" and "件"
        totalPages = math.floor(totalRestaurant/numResPerPage) + (1 if totalRestaurant % numResPerPage > 0 else 0) - 1 # -1 to exclude the first page
        page_links = []
...
# Scrape basic information (name, type, area, and link leading to the restaurant page) for each restaurant
def scrape_restaurants_links_name_type_area(page_link):
    ...
    if soup:
        ...
        attribute_mapping = {
            "name": ("a", lambda x: x.text.strip() if x else None), # .strip for removing leadning and trailing whitespaces
        ...
        # loop through extracted section of each restaurant in the specific page 
...
# Scrape detailed restaurant information (such as address, discount information etc.)
